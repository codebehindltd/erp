...
namespace HotelManagement.Data.Restaurant
{
    public class RestaurentPosDA : BaseService
    {
                        ...
                        if (status > 0)
                        {
                            using (DbCommand command = dbSmartAspects.GetStoredProcCommand("SaveKotBillDetailInfoForBilling_SP"))
                            {
                                foreach (KotBillDetailBO kbd in billDetails)
                                {
                                    ...
                                    dbSmartAspects.AddInParameter(command, "@BagWeight", DbType.Decimal, kbd.BagWeight);
                                    dbSmartAspects.AddInParameter(command, "@NoOfBag", DbType.Decimal, kbd.NoOfBag);
                ...
                using (DbTransaction transction = conn.BeginTransaction())
                {
                    ...
                    {                                                
                        using (DbCommand command = dbSmartAspects.GetStoredProcCommand("SaveRestaurentBillForSalesOrder_SP"))
                        {
                            ...
                            dbSmartAspects.AddOutParameter(command, "@SOrderId", DbType.Int32, sizeof(Int32));
        ...
        public RetailPosBillReturnBO GetSalesOrderInfoBySOIdForReport(int billId)
        {
            RetailPosBillReturnBO salesList = new RetailPosBillReturnBO();
            using (DbConnection conn = dbSmartAspects.CreateConnection())
            {
                using (DbCommand cmd = dbSmartAspects.GetStoredProcCommand("GetSalesOrderInfoBySOIdForReport_SP"))
                {
                    cmd.CommandTimeout = Convert.ToInt32(System.Configuration.ConfigurationManager.AppSettings["SqlCommandTimeOut"]);
                    dbSmartAspects.AddInParameter(cmd, "@SOrderId", DbType.Int32, billId);
                    ...
                    salesList.PosBillWithSalesReturn = ds.Tables[0].AsEnumerable().Select(r =>
                                new RetailPosBillWithSalesReturnBO
                                {
                                    ...
                                    BillId = r.Field<int>("BillId"),
                                    BillNumber = r.Field<string>("BillNumber"),
                                    BillDescription = r.Field<string>("BillDescription"),
                                    ReturnBillId = r.Field<int?>("ReturnBillId"),
                                    BillDate = r.Field<DateTime?>("BillDate"),
                                    CostCenter = r.Field<string>("CostCenter"),
                                    CashierName = r.Field<string>("CashierName"),
                                    ...
                                    RoundedAmount = r.Field<decimal?>("RoundedAmount"),
                                    RoundedGrandTotal = r.Field<decimal?>("RoundedGrandTotal"),
                                    IsInvoiceVatAmountEnable = r.Field<dynamic>("IsInvoiceVatAmountEnable").ToString() == "0"?false:true,
                                    IsInvoiceServiceChargeEnable = r.Field<dynamic>("IsInvoiceServiceChargeEnable").ToString() == "0"?false:true,
                                    InvoiceCitySDCharge = r.Field<decimal?>("InvoiceCitySDCharge"),
                                    KotId = r.Field<int>("KotId"),
                                    BearerId = r.Field<int?>("BearerId"),
                                    ReferenceKotId = r.Field<int?>("ReferenceKotId"),
                                    ...
                                    CashIncentive = r.Field<decimal?>("CashIncentive")
                                }).ToList();
