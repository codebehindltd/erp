<%@ Page Title="" Language="C#" MasterPageFile="~/Common/Innboard.Master" AutoEventWireup="true" EnableEventValidation="false"
    CodeBehind="frmReportGuestHouseInfo.aspx.cs" Inherits="HotelManagement.Presentation.Website.HotelManagement.Reports.frmReportGuestHouseInfo" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <script type="text/javascript">
        ...
        function ControlShowHideInfo()
        {
            ...
            else if ($("#<%=ddlReportType.ClientID %>").val() == "InHouseGroupList") {
                $('#CompanyInformationDiv').hide("slow");
                $('#GroupInformationDiv').show("slow");
                $('#ReferenceNumberDiv').hide("slow");
                ClearDate();
            }
            else if ($("#<%=ddlReportType.ClientID %>").val() == "CheckInList") {
                $('#CompanyInformationDiv').show("slow");
                $('#GroupInformationDiv').hide("slow");
                $('#ReferenceNumberDiv').hide("slow");
                ClearDate();
            }
            else if ($("#<%=ddlReportType.ClientID %>").val() == "CheckOutList") {
                $('#CompanyInformationDiv').hide("slow");
                $('#GroupInformationDiv').hide("slow");
                $('#ReferenceNumberDiv').hide("slow");
                ClearDate();
            }
            else if ($("#<%=ddlReportType.ClientID %>").val() == "ArrivalList") {
                $('#CompanyInformationDiv').show("slow");
                $('#GroupInformationDiv').hide("slow");
                $('#ReferenceNumberDiv').show("slow");
                ClearDate();
            }
            else if ($("#<%=ddlReportType.ClientID %>").val() == "ProbablyVacantList") {
                $('#CompanyInformationDiv').hide("slow");
                $('#GroupInformationDiv').hide("slow");
                $("#CheckInOutDateContainer").hide("slow");                
                ClearDate();
            }
            var reportName = $("#ContentPlaceHolder1_ddlReportType").val();
            var companyName = $("#ContentPlaceHolder1_ddlGuestCompany").val();
            ...
            <div class="form-horizontal">
                <div class="form-group">
                    <div class="col-md-2">
                        ...
                        <asp:Label ID="lblReportType" runat="server" class="control-label required-field" Text="Report Name"></asp:Label>
                    </div>
                    <div class="col-md-4">
                        <asp:DropDownList ID="ddlReportType" runat="server" CssClass="form-control" TabIndex="4">
                            <asp:ListItem Value="0">--- Please Select ---</asp:ListItem>
                            <asp:ListItem Value="ArrivalList">Expected Arrival Report</asp:ListItem>
                            <asp:ListItem Value="CheckInList">Checked-In Report</asp:ListItem>
                            <asp:ListItem Value="CheckOutList">Checked-Out Report</asp:ListItem>
                            <asp:ListItem Value="InHouseGustList">In House Guest Report</asp:ListItem>
                            <%--<asp:ListItem Value="InHouseGroupList">Group In House Guest Report</asp:ListItem>--%>
                            <asp:ListItem Value="ProbablyVacantList">Expected Departure (In House Guest) Report</asp:ListItem>
                        </asp:DropDownList>
                    </div>
                ...
                <div class="form-group" id="DateRangeContainerDiv">
                    <div class="col-md-2">
    ...
    <div id="ReportPanel" class="panel panel-default" style="display: none">
        <div class="panel-heading">
            Report:: Guest House Information
        </div>
        <div class="panel-body">
