...
{
    public partial class frmOnlineRoomReservation : BasePage
    {
        ...
        protected void gvRoomRegistration_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.DataItem != null)
            {
                ...
                else if (lblReservationStatusInfo.Text == "Active")
                {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                roomReservationBO.DateOut = hmUtility.GetDateTimeFromString(txtDateOut.Text, hfServerDateFormat.Value);
                ...
                if (btnSave.Text.Equals("Save"))
                {
                    ...
                    roomReservationBO.CreatedBy = 0;
                    Boolean status = roomReservationDA.SaveOnlineRoomReservationInfo(roomReservationBO, out tmpRoomTypeId, Session["ReservationDetailList"] as List<ReservationDetailBO>, complementaryItemBOList, out currentReservationNumber, paidServiceDetails, paidServiceDeleted);
                    if (status)
                    {
                        ...
                        EntityTypeEnum.EntityType.RoomReservationOnline.ToString(), tmpRoomTypeId,
                        ProjectModuleEnum.ProjectModule.FrontOffice.ToString(), hmUtility.GetEntityTypeEnumDescription(EntityTypeEnum.EntityType.RoomReservationOnline));
                ...
                else
                {
                    ...
                    {
                        retriveReservationDetailListBO = reservationDetailDA.GetOnlineReservationDetailByReservationId(roomReservationBO.ReservationId);
                    ...
                    //-------------------------------------------------------
                    string currentReservationNumber = String.Empty;
                    roomReservationBO.LastModifiedBy = 0;
        ...
        private void LoadServerDateFormat()
        {
            string innBoardDateFormat = string.Empty;
            ...
            hfServerDateFormat.Value = formats[0];
        }
        ...
        private void FillForm(int reservationId)
        {
            ...
            if (isGuestCountGreaterThenZero > 0)
            {
                ...
                ddlTitle.Text = guestInformationList[0].Title;
        ...
        public void LoadAirportPickupDrop(List<HotelReservationAireportPickupDropBO> airportPickupDropList)
        {
            ...
            List<HotelReservationAireportPickupDropBO> pickup = airportPickupDropList.Where(a => a.PickupDropType == "Pickup").ToList();
            List<HotelReservationAireportPickupDropBO> drop = airportPickupDropList.Where(a => a.PickupDropType == "Drop").ToList();

            if (pickup.Count == 1 && pickup[0].GuestId == 0)
            {
                hfAPId.Value = pickup[0].APDId.ToString();
                ddlAirportPickUp.SelectedValue = pickup[0].AirportPickUp;
                txtArrivalFlightName.Text = pickup[0].ArrivalFlightName;
                txtArrivalFlightNumber.Text = pickup[0].ArrivalFlightNumber;
                txtArrivalTime.Text = pickup[0].ArrivalTimeShow.ToString();
            }
            else if (pickup.Count >= 1)
            {
                ...
                foreach (HotelReservationAireportPickupDropBO bo in pickup)
                {
                    if (bo.GuestId != 0)
                    {
                        if (counter1 % 2 == 0)
            ...
            if (drop.Count == 1 && drop[0].GuestId == 0)
            {
                hfADId.Value = drop[0].APDId.ToString();
                ddlAirportDrop.SelectedValue = drop[0].AirportDrop;
                txtDepartureFlightName.Text = drop[0].DepartureFlightName;
                txtDepartureFlightNumber.Text = drop[0].DepartureFlightNumber;
                txtDepartureTime.Text = drop[0].DepartureTimeShow.ToString();
            }
            else if (drop.Count >= 1)
            {
                ...
                foreach (HotelReservationAireportPickupDropBO bo in drop)
                {
                    if (bo.GuestId != 0)
                    {
                        if (counter2 % 2 == 0)
        ...
        private void DeleteData(int pkId)
        {
            ...
            if (statusApproved)
            {
                hmUtility.CreateActivityLogEntity(ActivityTypeEnum.ActivityType.Delete.ToString(), EntityTypeEnum.EntityType.RoomReservationOnline.ToString(), pkId, ProjectModuleEnum.ProjectModule.FrontOffice.ToString(), hmUtility.GetEntityTypeEnumDescription(EntityTypeEnum.EntityType.RoomReservationOnline));
        ...
        public static string GetHTMLRoomGridView(List<RoomNumberBO> List, List<ReservationDetailBO> reservationDetailList)
        {
            ...
            strTable += "     <button type='button' onClick='javascript:return GetCheckedRoomCheckBox()' id='btnAddRoomId' class='btn btn-primary'> OK</button>";
            strTable += "     <button type='button' onclick='javascript:return CloseRoomDialog()' id='btnAddRoomId' class='btn btn-primary'> Cancel</button>";
            strTable += "</div>";
            return strTable;
        }
        ...
        {
            string strTable = "";
            strTable += "<table class='table table-bordered table-condensed table-responsive' cellpadding='4' id='ReservedGuestInformation'> " +
                        "<thead> <tr style='color: White; background-color: #44545E; font-weight: bold;'>";
            strTable += "<th align='left' scope='col'>Guest Name</th><th align='left' scope='col'>Email</th><th align='left' scope='col'>Room</th> " +
        ...
        [WebMethod(EnableSession = true)]
        public static ReturnInfo SaveReservation(RoomReservationBO RoomReservation, GuestInformationBO GuestDetailsInfo, List<ReservationDetailBO> RoomReservationDetail)
        {
            ...
            try
            {
                ...
                if (GuestDetailsInfo != null)
                    SaveGuestDetailsWithFInalSave(GuestDetailsInfo);

                if (RoomReservation.ReservationId == 0)
                {
                    RoomReservation.CreatedBy = 0;

                    rtnInfo.IsSuccess = roomReservationDA.SaveOnlineRoomReservationInfo(RoomReservation, out tmpReservationId, RoomReservationDetail, null, out currentReservationNumber, null, false);

                    if (rtnInfo.IsSuccess)
                    {
                        hmUtility.CreateActivityLogEntity(ActivityTypeEnum.ActivityType.Add.ToString(), EntityTypeEnum.EntityType.RoomReservationOnline.ToString(), tmpReservationId, ProjectModuleEnum.ProjectModule.FrontOffice.ToString(), hmUtility.GetEntityTypeEnumDescription(EntityTypeEnum.EntityType.RoomReservationOnline));
                ...
                //                                                reservationDetailDeletedList, AireportPickupDrop, AddedAirportPickupDrop, DeletedAirportPickupDrop, newlyAddedComplementaryItem,
        ...
        public static string SaveGuestInformationAsDetail(string reservationId, int prevGuestId, string isEdit, int IsEditFromAddMore, string title, string firstName, string lastName, string txtGuestName, string txtGuestEmail, string hiddenGuestId, string txtGuestDrivinlgLicense, string txtGuestDOB, string txtGuestAddress1, string txtGuestAddress2, string ddlProfessionId, string txtGuestCity, string ddlGuestCountry, string txtGuestNationality, string txtGuestPhone, string ddlGuestSex, string txtGuestZipCode, string txtNationalId, string txtPassportNumber, string txtPExpireDate, string txtPIssueDate, string txtPIssuePlace, string txtVExpireDate, string txtVisaNumber, string txtVIssueDate, int roomId, string selectedPreferenceId)
        {
            ...
            else
            {
                ...
                Boolean status = guestDA.UpdateGuestInformationNew(detailBO, "0", string.Empty, preferenList);
                if (status)
                {
                    hmUtility.CreateActivityLogEntity(ActivityTypeEnum.ActivityType.Edit.ToString(), EntityTypeEnum.EntityType.GuestInformation.ToString(), detailBO.GuestId,
                         ProjectModuleEnum.ProjectModule.FrontOffice.ToString(), hmUtility.GetEntityTypeEnumDescription(EntityTypeEnum.EntityType.GuestInformation));
        ...
        private static void SaveGuestDetailsWithFInalSave(GuestInformationBO guestInfo)
        {
            bool status = false;
            HMUtility hmUtility = new HMUtility();
            RoomReservationDA resDA = new RoomReservationDA();
            ...
            if (guestInfo.GuestId == 0)
            {
                ...
                {
                    status = resDA.SaveTemporaryGuestOnline(guestInfo, guestInfo.tempOwnerId.ToString());
                    if (status)
                    {
                        hmUtility.CreateActivityLogEntity(ActivityTypeEnum.ActivityType.Add.ToString(), EntityTypeEnum.EntityType.GuestInformationOnline.ToString(), guestInfo.GuestId,
                                ProjectModuleEnum.ProjectModule.FrontOffice.ToString(), hmUtility.GetEntityTypeEnumDescription(EntityTypeEnum.EntityType.GuestInformationOnline));
                    }
                ...
                else
                {
                    ...
                    if (status)
                    {
                        hmUtility.CreateActivityLogEntity(ActivityTypeEnum.ActivityType.Edit.ToString(), EntityTypeEnum.EntityType.GuestInformation.ToString(), guestInfo.GuestId,
                                ProjectModuleEnum.ProjectModule.FrontOffice.ToString(), hmUtility.GetEntityTypeEnumDescription(EntityTypeEnum.EntityType.GuestInformation));
                        if (preferenList.Count > 0)
                        {
                            foreach (var item in preferenList)
        ...
        public static string PerformSaveRoomDetailsInformationByWebMethod(bool isEdit, string hfSelectedRoomNumbers, string hfSelectedRoomId, string txtUnitPriceHiddenField, string txtRoomRate, string txtRoomId, string prevRoomTypeId, string ddlRoomTypeId, string ddlRoomTypeIdText, string lblHiddenId, string txtDiscountAmount, string ddlCurrency, string ddlDiscountType, string ddlReservedMode)
        {
            ...
            if (Rooms[0].Length == 0)
            {
                ...
                for (int i = 0; i < roomQuantity; i++)
                {
                    ...
                    detailBO.RoomNumber = "Unassigned";
                    detailBO.ReservationDetailId = 0;
                    reservationDetailListBO.Add(detailBO);
                }
            }
            else
        ...
        public static ReturnInfo DeleteReservationRecord(int pkId)
        {
            ...
            try
            {
                ...
                if (status == true)
                {
                    ...
                    hmUtility.CreateActivityLogEntity(ActivityTypeEnum.ActivityType.Delete.ToString(), EntityTypeEnum.EntityType.RoomReservation.ToString(),pkId,
                 ProjectModuleEnum.ProjectModule.FrontOffice.ToString(), hmUtility.GetEntityTypeEnumDescription(EntityTypeEnum.EntityType.RoomReservation)+".Deleted By ReservationId");
        ...
        [WebMethod(EnableSession = true)]
        public static string PerformGetRoomAvailableChecking(string serverDateFormat, string RoomTypeId, string fromDate, string toDate)
        {
            ...
            if (!string.IsNullOrWhiteSpace(fromDate))
            {
                startDate = hmUtility.GetDateTimeFromString(fromDate, serverDateFormat);
            }
            else
            {
                fromDate = hmUtility.GetStringFromDateTime(dateTime);
            ...
            if (!string.IsNullOrWhiteSpace(toDate))
            {
                endDate = hmUtility.GetDateTimeFromString(toDate, serverDateFormat).AddDays(-1);
            }
            else
            {
                toDate = hmUtility.GetStringFromDateTime(dateTime);
        ...
        public static string MultipleAireportPickupDropInfo(int reservationId)
        {
            ...
            strTable += "<th align='left' scope='col'>Guest Name</th><th align='left' scope='col'>Arrival Flight Name</th><th align='left' scope='col'>Action</th></thead>";
            strTable += "<tbody>";
            ...
            foreach (GuestInformationBO dr in guestInfoList)
            {
                ...
                strTable += "<td align='left' style='width: 138px'>" + dr.ArrivalFlightName + "</td>";
        ...
        [WebMethod]
        public static RoomTypeBO PerformFillFormActionByTypeId(string innBoardDateFormat, int EditId, string RoomTypeId, string fromDate, string toDate)
        {
            RoomTypeBO roomTypeBO = new RoomTypeBO();
            RoomTypeDA roomTypeDA = new RoomTypeDA();
            ...
            //List<RoomNumberBO> list = new List<RoomNumberBO>();
            int _reservationId = 0;
            if (HttpContext.Current.Session["_onlineRoomReservationId"] != null)
            {
                _reservationId = Int32.Parse(HttpContext.Current.Session["_onlineRoomReservationId"].ToString());
            }
            DateTime dateTime = DateTime.Now;
            DateTime StartDate = dateTime;
            DateTime EndDate = dateTime;
            if (!string.IsNullOrWhiteSpace(fromDate))
            {
                StartDate = hmUtility.GetDateTimeFromString(fromDate, innBoardDateFormat);
            }
            else
            {
                fromDate = hmUtility.GetStringFromDateTime(dateTime);
            ...
            if (!string.IsNullOrWhiteSpace(toDate))
            {
                EndDate = hmUtility.GetDateTimeFromString(toDate, innBoardDateFormat);
            }
            else
            {
                toDate = hmUtility.GetStringFromDateTime(dateTime);
                EndDate = hmUtility.GetDateTimeFromString(toDate, innBoardDateFormat).AddDays(1);
            }
            ...
            roomTypeBO.AvailableRoomCount = availableRoomQty;
