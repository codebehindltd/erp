...
{
    public partial class LeaveInformation : BasePage
    {
        ...
        public static ReturnInfo GetLeaveApproval(int id, string action, string cencelReason)
        {
            ...
            UserInformationBO userInformationBO = new UserInformationBO();
            userInformationBO = hmUtility.GetCurrentApplicationUserInfo();
            if (action == HMConstants.ApprovalStatus.Checked.ToString())
            {
                leaveInformationBO.LeaveStatus = HMConstants.ApprovalStatus.Checked.ToString();
                leaveInformationBO.CheckedBy = userInformationBO.UserInfoId;

                rtninf.AlertMessage = CommonHelper.AlertInfo(AlertMessage.Checked, AlertType.Success);
            }
            else if (action == HMConstants.ApprovalStatus.Approved.ToString())
            {
                leaveInformationBO.LeaveStatus = HMConstants.ApprovalStatus.Approved.ToString();
                leaveInformationBO.ApprovedBy = userInformationBO.UserInfoId;
            ...
            else if (action == HMConstants.ApprovalStatus.Cancel.ToString())
            {
                ...
                leaveInformationBO.CancelReason = cencelReason;
        ...
        public static GridViewDataNPaging<LeaveInformationBO, GridPaging> LoadLeaveInformation(int isAdminUser, string fromDate, string toDate, int empId, string searchStatus, int gridRecordsCount, int pageNumber, int isCurrentOrPreviousPage)
        {
            ...
            if (userInformationBO.EmpId != 0)
            {
                ...
                Boolean isWorkHandoverEnableForLeaveApplication = false;
                HMCommonSetupDA commonSetupDA = new HMCommonSetupDA();
                HMCommonSetupBO isWorkHandoverEnableForLeaveApplicationBO = new HMCommonSetupBO();
                isWorkHandoverEnableForLeaveApplicationBO = commonSetupDA.GetCommonConfigurationInfo("IsWorkHandoverEnableForLeaveApplication", "IsWorkHandoverEnableForLeaveApplication");

                if (isWorkHandoverEnableForLeaveApplicationBO != null)
                {
                    if (isWorkHandoverEnableForLeaveApplicationBO.SetupId > 0)
                    {
                        if (Convert.ToInt32(isWorkHandoverEnableForLeaveApplicationBO.SetupValue) > 0)
                        {
                            isWorkHandoverEnableForLeaveApplication = true;
                        }
                    }
                }

                List<LeaveInformationBO> employeeLeave = new List<LeaveInformationBO>();
                if (isWorkHandoverEnableForLeaveApplication)
                {
                    employeeLeave = entityDA.GetEmpLeaveInformationByEmpIdForGridPaging(isAdminUser, userInformationBO.UserInfoId, searchEmployeeId, searchStatus, leaveType, leaveMode, leaveFromDate, leaveToDate, userInformationBO.GridViewPageSize, pageNumber, out totalRecords).Where(x => x.WorkHandoverStatus == "Accepted").ToList();
                }
                else
                {
                    employeeLeave = entityDA.GetEmpLeaveInformationByEmpIdForGridPaging(isAdminUser, userInformationBO.UserInfoId, searchEmployeeId, searchStatus, leaveType, leaveMode, leaveFromDate, leaveToDate, userInformationBO.GridViewPageSize, pageNumber, out totalRecords);
                }
