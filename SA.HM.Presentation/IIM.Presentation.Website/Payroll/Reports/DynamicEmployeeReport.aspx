<%@ Page Title="" Language="C#" MasterPageFile="~/Common/Innboard.Master" AutoEventWireup="true" CodeBehind="DynamicEmployeeReport.aspx.cs" Inherits="HotelManagement.Presentation.Website.Payroll.Reports.DynamicEmployeeReport" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#btnTransferRight").click(function () {
                ...
                if (options.length > 0) {
                    if (!checkCol) {
                        ...
                        selectedArrText.push(options[0].text);
            ...
            $("#btnDelete").click(function () {
                ...
                if (options.length > 0) {
                    ...
                    for (var i = 0; i < options.length; i++) {
                        $("#ContentPlaceHolder1_lstAvailableFields").append($(options[i]));
        ...
        function CheckValidation(IsSave) {
            ...
            var sequenceArrText = new Array;
            for (var i = 0; i < list.options.length; i++) {
                sequenceArr.push(list.options[i].value);
            }
            for (var i = 0; i < list.options.length; i++) {
                sequenceArrText.push(list.options[i].text);
            }
            <%--if (fromDate == "") {
            ...
            PageMethods.SaveAndGenerateReport(IsSave, selectedArr, joinTable, sequenceArr, selectedArrText, sequenceArrText, fromDate, toDate, reportType, reportName, filter, sequence, OnSucceed, OnFailed);
        }
        function OnSucceed(result) {
            var reportName = $("#<%=txtReportName.ClientID %>").val();
            var iframeid = 'IframeReport';
            var url = "/SalesAndMarketing/Reports/CrmDynamicIframe.aspx?name=" + reportName;
            ...
            if (result[0].RawColumns.length > 0) {
                ...
                $.each(result[0].RawColumns, function (i, item) {
                    ...
                    $('#ContentPlaceHolder1_lstAvailableFields').append($('<option>', {
                        value: "EMP" + "." + item.ColumnName,
                        text: result[0].Rearranged[i].ColumnName
                    }));
                ...
                <div class="form-group" style="display: none">
                    ...
                    <div class="col-md-4">
                        <asp:DropDownList ID="ddlSegment" runat="server" CssClass="form-control" TabIndex="2">
                            <asp:ListItem Value="PayrollEmployee">Payroll Employee</asp:ListItem>
                        </asp:DropDownList>
                    </div>
                </div>
