...
{
    public partial class frmReportTPSplitBillInfo : System.Web.UI.Page
    {
        ...
        private void ForwordReportProcessing()
        {
            ...
            string discountString = Request.QueryString["disc"];
            string itmString = Request.QueryString["itm"];
            ...
            if ((queryStringId.Split('~')[0]).Length > 0)
            {
                ...
                if (!string.IsNullOrEmpty(queryStringId))
                {
                    ...
                    if (costCentreTabBO != null)
                    {
                        ...
                        Boolean status = entityDA.UpdateKotBillDetailPrintFlagByOrderSubmitBillId(billID);
        ...
        private void ReportProcessing(string reportName)
        {
            ...
            if ((itmString.Split('~')).Length > 0)
            {
                ...
                if (!string.IsNullOrEmpty(queryStringId))
                {
                    ...
                    RestaurantBillBO bill = new RestaurantBillBO();
                    ...
                    bill = rda.GetBillInfoByBillId(billID);

                    decimal netSales = 0;
                    decimal discount = Convert.ToDecimal(discountString);
                    ...
                    string[] itemIdList = itmString.Split(',');
                    ...
                    RestaurantBillDistribution d = new RestaurantBillDistribution();
                    ...
                    foreach (RestaurantBillReportBO invoiceBO in restaurantBillInvoice)
                    {
                        ...
                        d = rda.GetRestaurantBillDistribution("ItemWise", invoiceBO.CostCenterId, invoiceBO.ItemId, invoiceBO.DiscountedAmount, bill.DiscountType, discount,
                                                             bill.IsInvoiceVatAmountEnable, bill.IsInvoiceServiceChargeEnable, bill.IsInvoiceCitySDChargeEnable, bill.IsInvoiceAdditionalChargeEnable, false);

                        invoiceBO.ServiceCharge = d.ServiceCharge;
                        invoiceBO.CitySDCharge = d.SDCityCharge;
                        invoiceBO.VatAmount = d.VatAmount;
                        invoiceBO.AdditionalCharge = d.AdditionalCharge;
                        invoiceBO.DiscountAmount = d.DiscountAmount;
                        invoiceBO.DiscountedAmount = invoiceBO.DiscountedAmount - d.DiscountAmount;
                        ...
                        {
                            invoiceBO.NetAmount = (netSales - invoiceBO.ServiceCharge - invoiceBO.VatAmount - invoiceBO.CitySDCharge - invoiceBO.AdditionalCharge);
                            invoiceBO.TotalSales = netSales; // invoiceBO.NetAmount;
