...
{
    public partial class frmKotBillMaster : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            TableInformationDiv.Visible = true;
            if (!IsPostBack)
            {
                this.IsItemNameAutoSearchWithCode();
                this.LoadCompanyInformation();
                this.IsRestaurantPaxConfirmationEnable();
            ...
            hfIsItemCanEditDelete.Value = (currentUserInformationBO.IsItemCanEditDelete == true ? "1" : "0");
            hfIsUserHasEditDeleteAccess.Value = (currentUserInformationBO.IsItemCanEditDelete == true ? "1" : "0");
            ...
            if (!string.IsNullOrEmpty(queryStringId))
            {
                ...
                {
                    if (queryStringId.Split(':').Count() < 3)
                    {
                        this.BillCreateForTable(queryStringId.Split(':')[1].ToString(), "0");
                    }
                    else if (queryStringId.Split(':').Count() > 2)
                    {
                        this.BillCreateForTable(queryStringId.Split(':')[1].ToString(), queryStringId.Split(':')[2].ToString());
                    }
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (matrixBO.CategoryId > 0)
                    {
                        ...
                        {
                            Response.Redirect("/POS/frmKotBillMaster.aspx?Kot=RestaurantItemCategory:0");
                        }
                        else
                        {
                            Response.Redirect("/POS/frmKotBillMaster.aspx?Kot=RestaurantItemCategory:" + (matrixBO.AncestorId).ToString());
                        }
                    }
                }
            }
        ...
        protected void btnOrderSubmit_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                if (files.Count > 0)
                {
                    ...
                    {
                        Boolean logStatus = hmUtility.CreateActivityLogEntity(ActivityTypeEnum.ActivityType.Edit.ToString(), EntityTypeEnum.EntityType.RestaurantItem.ToString(), 0,
                        ProjectModuleEnum.ProjectModule.RestaurantManagement.ToString(), hmUtility.GetEntityTypeEnumDescription(EntityTypeEnum.EntityType.RestaurantItem)+"Order Submitted");
        ...
        protected void btnKotRePrintPreview_Click(object sender, EventArgs e)
        {
            ...
            tableBO = tableDA.GetRestaurantTableInfoByTableId(tableId);
            if (tableBO.TableId > 0)
            {
                string url = "/POS/Reports/frmReportKotBillInfo.aspx?kotId=" + kotId.ToString() + "&tno=" + tableBO.TableNumber + "&kbm=" + "kotm" + "&isrp=" + "rp";
                string s = "window.open('" + url + "', 'popup_window', 'width=750,height=680,left=300,top=50,resizable=yes');";
                ClientScript.RegisterStartupScript(this.GetType(), "script", s, true);
            }
        }
        //************************ User Defined Function ********************//
        private void IsItemNameAutoSearchWithCode()
        {
            hfIsItemNameAutoSearchWithCode.Value = "0";
            HMCommonSetupBO isItemNameAutoSearchWithCodeBO = new HMCommonSetupBO();
            HMCommonSetupDA commonSetupDA = new HMCommonSetupDA();

            isItemNameAutoSearchWithCodeBO = commonSetupDA.GetCommonConfigurationInfo("IsItemNameAutoSearchWithCode", "IsItemNameAutoSearchWithCode");
            if (isItemNameAutoSearchWithCodeBO != null)
            {
                if (isItemNameAutoSearchWithCodeBO.SetupId > 0)
                {
                    if (isItemNameAutoSearchWithCodeBO.SetupValue == "1")
                    {
                        hfIsItemNameAutoSearchWithCode.Value = "1";
                    }
                }
            }
        }
        ...
        private void IsRestaurantPaxConfirmationEnable()
        {
            ...
            hfIsRestaurantPaxConfirmationEnable.Value = "0";
            if (isRestaurantPaxConfirmationEnableBO != null)
            {
                hfIsRestaurantPaxConfirmationEnable.Value = isRestaurantPaxConfirmationEnableBO.SetupValue;
            }
        }
        ...
        {
            TableInformationDiv.Visible = false;
            tableallocation.Visible = true;
            RestaurantItem.Visible = false;
            ...
            string topPart = @"<div id='legendContainerForRestaurant' class='row' style='padding-left:20px;'>
            ...
            string topTemplatePart = @"<div class='row'>
            ...
            for (int iTableNumber = 0; iTableNumber < entityListBO.Count; iTableNumber++)
            {
                ...
                {
                    if (hfIsRestaurantPaxConfirmationEnable.Value == "0")
                    {
                    ...
                    {
                        Content0 = @"<a href='/POS/frmKotBillMaster.aspx?Kot=BillCreateForTable:" + entityListBO[iTableNumber].TableId;
                        Content1 = @"' onclick='javascript:return PaxConfirmation(1, " + entityListBO[iTableNumber].TableId + ");' id=a" + entityListBO[iTableNumber].TableId + "><div class='NotDraggable RestaurantTableAvailableDiv' style='width:" + entityListBO[iTableNumber].TableWidth + "px; height:" + entityListBO[iTableNumber].TableHeight + "px; top:" + entityListBO[iTableNumber].YCoordinate + "px; left:" + entityListBO[iTableNumber].XCoordinate + "px;' id='" + entityListBO[iTableNumber].TableManagementId + "'>";
                        Content2 = @"<div class='RestaurantTableAvailableDiv'>
                                        </div>
                                        <div class='TableNumberDiv'>" + entityListBO[iTableNumber].TableNumber + "</div></div></a>";
                    }
            ...
            this.ltlRoomTemplate2.Text = fullContent;
        }
        private void LoadRestaurantItemCategory(int categoryId)
        {
            tableallocation.Visible = false;
            ...
            for (int iItemCategory = 0; iItemCategory < roomNumberListWithoutZeroBO.Count; iItemCategory++)
            {
                ...
                if (roomNumberListWithoutZeroBO[iItemCategory].ChildCount.ToString() == "1")
                {
                    Content1 = @"<div class='DivRestaurantItemContainer'>
                                        <a href='/POS/frmKotBillMaster.aspx?Kot=RestaurantItem:" + roomNumberListWithoutZeroBO[iItemCategory].CategoryId;
                }
                else
                {
                    Content1 = @"<div class='DivRestaurantItemContainer'>
                                        <a href='/POS/frmKotBillMaster.aspx?Kot=RestaurantItemCategory:" + roomNumberListWithoutZeroBO[iItemCategory].CategoryId;
                }
            ...
            fullContent += fullItemContent + subContent;
        ...
        private void LoadRestaurantItem(string queryString)
        {
            ...
            if (!string.IsNullOrWhiteSpace(queryString))
            {
                ...
                for (int iItem = 0; iItem < roomNumberListBO.Count; iItem++)
                {
                    ...
                    //string Content4 = @");'  src='" + roomNumberListBO[iItem].ImageName;
                    //string Content5 = @"' /></div><div class='ItemNameDiv'>" + roomNumberListBO[iItem].Name + "</div></div>";

                    string Content1 = @"<div class='DivRestaurantItemContainer'>";
                    string Content2 = @"<div class='RestaurantItemDiv'><img class='ItemImageSize' id='ContentPlaceHolder1_img" + roomNumberListBO[iItem].ItemId + "'";
                    string Content3 = string.Format("onclick=\"javascript:return PerformAction({0}, '{1}')\"", roomNumberListBO[iItem].ItemId, roomNumberListBO[iItem].Name);
                    //string Content2 = @"<div class='RestaurantItemDiv'><img class='ItemImageSize' ID='ContentPlaceHolder1_img" + roomNumberListBO[iItem].ItemId + "' onclick=\"AmiGhada(0, '1')\" src='" + roomNumberListBO[iItem].ImageName + "'/>";
                    string Content4 = "src='" + roomNumberListBO[iItem].ImageName;
                    string Content5 = @"' /></div>
                                        <div class='ItemNameDiv'>" + roomNumberListBO[iItem].Name + "</div></div>";
        ...
        private void BillCreateForTable(string queryString, string paxQuantity)
        {
            if (!string.IsNullOrWhiteSpace(queryString))
            {
                ...
                if (hfIsRestaurantPaxConfirmationEnable.Value == "1")
                {
                    if (paxQuantity != "0")
                    {
                        entityBO.PaxQuantity = !string.IsNullOrWhiteSpace(paxQuantity) ? Convert.ToInt32(paxQuantity) : 1;
                    }
                }
                else
                {
                    entityBO.PaxQuantity = 1;
                }
        ...
        public static string GenerateTableWiseItemGridInformation(int costCenterId, int tableId, int kotId)
        {
            ...
            if (userInformationBO != null)
            {
                ...
                foreach (KotBillDetailBO dr in files)
                {
                    ...
                    if (!dr.PrintFlag)
                    {
                        strTable += "<img id='imgedit" + counter + "' src='../Images/edit.png' title='Edit'  onClick='javascript:return AddNewItem(" + dr.KotDetailId + "," + dr.ItemId + "," + counter + "," + (dr.IsItemEditable == true ? 1 : 0) + ")' alt='Edit Information' border='0' />";
                        strTable += "&nbsp;<img id='imgdelete" + counter + "' src='../Images/delete.png' title='Delete'  onClick='javascript:return PerformDeleteAction(" + dr.KotDetailId + "," + dr.KotId + "," + dr.ItemId + "," + counter + ")' alt='Delete Information' border='0' />";
                    ...
                    strTable += "<td align='left' style='display:none;'>" + (isChangedExist == false ? "1" : "0") + "</td>";
                    strTable += "</tr>";
        ...
        public static ReturnInfo DeleteData(int kotDetailsId, int kotId, int itemId)
        {
            ...
            try
            {
                ...
                if (status)
                {
                    Boolean logStatus = hmUtility.CreateActivityLogEntity(ActivityTypeEnum.ActivityType.Delete.ToString(), EntityTypeEnum.EntityType.RestaurantItem.ToString(), itemId,
                                    ProjectModuleEnum.ProjectModule.RestaurantManagement.ToString(), hmUtility.GetEntityTypeEnumDescription(EntityTypeEnum.EntityType.RestaurantItem));
        ...
        [WebMethod]
        public static bool UpdateTablePaxInformation(int costCenterId, int paxQuantity)
        {
            KotBillMasterBO entityBO = new KotBillMasterBO();
            ...
            if (costCenterId > 0)
            {
                ...
                if (tableBO.TableId > 0)
                {
                    status = entityDA.UpdateKotBillMasterPaxInfo(kotId, costCenterId, tableBO.TableId, paxQuantity);
                }
            }
        ...
        public static ReturnInfo LoadOccupiedPossiblePath(string KotNTableNumber, string pageTitle)
        {
            ...
            KotBillDetailDA entityDA = new KotBillDetailDA();
            List<KotBillDetailBO> files = entityDA.GetKotBillDetailInfoByKotNBearerId(Convert.ToInt32(costCenterNTableNumber[0]), "RestaurantTable", Convert.ToInt32(costCenterNTableNumber[1]), kotId);
            var v = files.Where(s => s.PrintFlag == true).ToList();

            bool isEmptyKot = masterDa.GetIsKotItemEmptyOrNotByKotId(kotId);
            bool isBillReSettlement = masterDa.GetIsKotIsSettledOrNotByKotId(kotId, Convert.ToInt32(costCenterNTableNumber[1]));
            ...
            strTable += " onclick=\" location.href='" + "/Restaurant/frmKotBillMaster.aspx?Kot=BillCreateForTable:" + costCenterNTableNumber[1] + "';\"  />";
            strTable += "</div>";

            strTable += "<div style='float:left;padding-left:10px; padding-bottom:20px;'>";
            strTable += "<input type='button' style='width:115px' value='" + "Table Change" + "' class='btn btn-primary'";
            strTable += " onclick=\"return OpenTableShiptPanel('" + costCenterNTableNumber[0] + "', '" + costCenterNTableNumber[1] + "' );\"  />";
            strTable += "</div>";

            if (isWMDeletePermission)
            {
                strTable += "<div style='float:left;padding-left:10px;'>";
                strTable += "<input type='button' style='width:115px' value='" + "Table Clean" + "' class='btn btn-primary'";
            ...
            {
                strTable += "<div style='float:left;padding-left:10px; padding-bottom:20px;'>";
                strTable += "<input type='button' style='width:115px' value='" + "Table Bill" + "' class='btn btn-primary'";
                strTable += " onclick=\"return BillEmptyOrNot(" + costCenterNTableNumber[1] + ");\"  />";
                strTable += "</div>";
            }
            else
            {
            ...
            strTable += " onclick=\" location.href='" + "/Restaurant/frmKotBillMaster.aspx?Kot=KotReprint:" + costCenterNTableNumber[1] + "';\"  />";
            strTable += "</div>";
            ...
            if (tableList.Count() > 0)
            {
                ...
                strTable += "<div class='form-horizontal'>";
                strTable += "<div class='form-group'>";
                strTable += "<div class='col-md-3'>";
                strTable += "Available Table";
                strTable += "</div>";
                strTable += "<div class='col-md-3'>" + availAbleTable;
                strTable += "</div>";
            }
            strTable += "<div class='col-md-3'>";
            strTable += "<input type='button' value='" + "Change" + "' class='btn btn-primary'";
            strTable += " onclick=\"return UpdateTableShift('" + costCenterNTableNumber[0] + "', '" + costCenterNTableNumber[1] + "' );\"  />";
            strTable += "</div></div></div></div></div></div>";

            strTable += "<div class='row'>";
            strTable += "<div style='font-weight: bold; display:none; height:60px; border:1px solid #ccc;' id='TableCleanInfo'>";
            strTable += "<div style='float:left;padding-left:28px; padding-bottom:5px; margin-top:10px; margin-bottom:10px;'>";
            strTable += "<div style='float:left;padding-left:28px; padding-bottom:5px; margin-top:10px;'>";
            strTable += "<input type='button' value='" + "Clean Table" + "' class='btn btn-primary'";
            strTable += " onclick=\"return UpdateTableStatus('" + costCenterNTableNumber[0] + "', '" + costCenterNTableNumber[1] + "' );\"  />";
            strTable += "</div></div></div></div>";
        ...
        public static bool UpdateRestaurantTableStatus(int costCenterId, int tableId, int statusId)
        {
            ...
            if (updateStatus)
            {
                Boolean logStatus = hmUtility.CreateActivityLogEntity(ActivityTypeEnum.ActivityType.Edit.ToString(), EntityTypeEnum.EntityType.RestaurantTable.ToString(), tableId,
                                        ProjectModuleEnum.ProjectModule.RestaurantManagement.ToString(), hmUtility.GetEntityTypeEnumDescription(EntityTypeEnum.EntityType.RestaurantTable)+"Cleaned");
            }
        ...
        public static bool UpdateRestaurantTableShift(int costCenterId, int oldTableId, int newTableId)
        {
            ...
            if (updateStatus)
            {
                Boolean logStatus = hmUtility.CreateActivityLogEntity(ActivityTypeEnum.ActivityType.Edit.ToString(), EntityTypeEnum.EntityType.RestaurantTable.ToString(), oldTableId,
                                                   ProjectModuleEnum.ProjectModule.RestaurantManagement.ToString(),"Table Shifted");
            }
        ...
        public static ReturnInfo SaveKotSpecialRemarks(List<RestaurantKotSpecialRemarksDetailBO> kotSRemarksDetail, List<RestaurantKotSpecialRemarksDetailBO> kotSRemarksDetailTobeDelete, int kotDetailId)
        {
            ...
            if (status)
            {
                Boolean logStatus = hmUtility.CreateActivityLogEntity(ActivityTypeEnum.ActivityType.Add.ToString(), EntityTypeEnum.EntityType.RestaurantItemRemarks.ToString(), tempItemId,
                                    ProjectModuleEnum.ProjectModule.RestaurantManagement.ToString(), EntityTypeEnum.EntityType.RestaurantItemRemarks.ToString());
                rtninf.IsSuccess = true;
        ...
        public void PrintReportKot(PrinterInfoBO files, List<KotBillDetailBO> entityBOList, Boolean IsReprint)
        {
            try
            {
                ...
                DateTime dateTime = DateTime.Now;
                //string kotDate = String.Format("{0:f}", dateTime);
                string kotDate = hmUtility.GetDateTimeStringFromDateTime(dateTime); //hmUtility.GetStringFromDateTime(dateTime) + " " + dateTime.ToString("hh:mm:ss tt");
                ...
                {
                    if (kotBillMasterBO.SourceName == "RestaurantTable")
                    {
                        //if (string.IsNullOrWhiteSpace(txtTableNumberInformation.Text))
                        //{
                        RestaurantTableDA tableDA = new RestaurantTableDA();
                        RestaurantTableBO tableBO = new RestaurantTableBO();
                        tableBO = tableDA.GetRestaurantTableInfoByTableId(kotBillMasterBO.SourceId);
                        if (tableBO.TableId > 0)
                        {
                            txtTableNumberInformation.Text = tableBO.TableNumber;
                        }
                        //}

                        if (reportTitle == "KOT (Reprint)")
                        {
