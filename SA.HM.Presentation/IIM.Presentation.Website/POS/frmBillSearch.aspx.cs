...
{
    public partial class frmBillSearch : BasePage
    {
        ...
        public static GridViewDataNPaging<RestaurantBillBO, GridPaging> SearchBillAndLoadGridInformation(string srcFromDate, string srcToDate, string billNo, string customerInfo, string remarks, string costCenterId, int gridRecordsCount, int pageNumber, int isCurrentOrPreviousPage)
        {
            ...
            reservationInfoList = rrDA.GetRestaurantBillInfoBySearchCriteriaForpagination(fromDate, toDate, billNo, customerInfo, remarks, userInfoId, costCenter, userInformationBO.GridViewPageSize, pageNumber, out totalRecords).Where(x => x.IsBillSettlement != false).ToList();
