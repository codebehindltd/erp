...
{
    public partial class frmBillReSettlement : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                LoadLocalCurrencyId();
                if (Request.QueryString["ot"] == "br") //HttpContext.Current.Session["KotResettlementBill"] != null
                {
                    PaymentResume();
        ...
        private void LoadRestaurantOrder()
        {
            ...
            if (billmasterforCheck != null)
            {
                kotIdNew = billmasterforCheck.KotId;
            }
            else
            {
                ...
                billmaster.BearerId = userInformationBO.BearerId;
                ...
                //HttpContext.Current.Session["KotResettlementBill"] = billmaster;
            }
        ...
        private void PaymentResume()
        {
            ...
            //kotBillMaster = (KotBillMasterBO)Session["KotResettlementBill"];
            ...
            //Session["RestaurantKotBillResumeForResettlement"] = paymentResume;
        ...
        protected void btnPrintPreview_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(queryStringId))
                {
                    ...
                    FileStream fs = new FileStream(HttpContext.Current.Server.MapPath("~/PrintDocument/" + fileName), FileMode.Create, FileAccess.ReadWrite);
                    ...
                    PdfWriter writer = PdfWriter.GetInstance(document, new FileStream(
                       HttpContext.Current.Server.MapPath("~/PrintDocument/" + fileNamePrint), FileMode.Create, FileAccess.ReadWrite));
                    document.Open();
        ...
        [WebMethod]
        public static KotBillDetailBO SaveIndividualItemDetailInformation(int isItemCanEditDelete, int costCenterId, int kotId, int itemId, decimal itemQty)
        {
                ...
                if (costCentreTabBO.CostCenterId > 0)
                {
                    ...
                    foreach (KotBillDetailBO dr in files)
                    {
                        ...
                        if (dr.KotDetailId > 0)
                        {
                            if (!dr.PrintFlag)
                            {
                                strTable += "<td data-title='Action' align='center' style='width: 15%;'><button type='button' class='TransactionalButton btn btn-primary btn-small' data-dismiss='alert' onClick='javascript:return AddNewItem(" + dr.KotDetailId + "," + dr.KotId + "," + dr.ItemId + "," + 0 + "," + (dr.IsItemEditable == true ? 1 : 0) + ")' alt='Action Decider' " + (ownKot != dr.KotId ? actionDisabled : string.Empty) + ">Option</button></td>";
                            }
                            else
                            {
                                if (!isChangedExist)
                                {
                                    strTable += "<td data-title='Action' align='center' style='width: 15%;'><button type='button' class='TransactionalButton btn btn-success  btn-small' data-dismiss='alert' onClick='javascript:return AddNewItem(" + dr.KotDetailId + "," + dr.KotId + "," + dr.ItemId + "," + 1 + "," + (dr.IsItemEditable == true ? 1 : 0) + ")' alt='Action Decider' " + (ownKot != dr.KotId ? actionDisabled : string.Empty) + ">Option</button></td>";
                                }
                                else
                                {
                                    strTable += "<td data-title='Action' align='center' style='width: 15%;'><button type='button' class='TransactionalButton btn btn-primary  btn-small' data-dismiss='alert' onClick='javascript:return AddNewItem(" + dr.KotDetailId + "," + dr.KotId + "," + dr.ItemId + "," + 1 + "," + (dr.IsItemEditable == true ? 1 : 0) + ")' alt='Action Decider' " + (ownKot != dr.KotId ? actionDisabled : string.Empty) + ">Option</button></td>";
                                }
        ...
        [WebMethod]
        public static KotBillDetailBO UpdateIndividualItemDetailInformation(string userId, string userIdAuthorisedPassword, int isItemCanEditDelete, string updateType, int costCenterId, int editId, decimal quantity, string updatedContent)
        {
            KotBillDetailBO entityBO = new KotBillDetailBO();
            KotBillDetailDA entityDA = new KotBillDetailDA();
        ...
        [WebMethod]
        public static string LoadAlreadyOccupiedTable(int costCenterId, int tableId)
        {
