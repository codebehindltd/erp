<%@ Page Title="" Language="C#" MasterPageFile="~/Common/Innboard.Master" AutoEventWireup="true"
    EnableEventValidation="false" CodeBehind="frmPMProductPO.aspx.cs" Inherits="HotelManagement.Presentation.Website.PurchaseManagment.frmPMProductPO" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#ContentPlaceHolder1_ddlPRNumber").select2({
                tags: "true",
                placeholder: "--- All ---",
                allowClear: true,
                width: "99.75%"
        ...
        $(document).ready(function () {
            ...
            $('#btnAdd').click(function () {
                ...
                if (editedItemId != "") {
                    var editedItemId = $.trim($(editedItem).find("td:eq(10)").text());

                    if (editedItemId != itemId) {
                        duplicateItemId = $("#POrderProductGrid tbody tr").find("td:eq(14):textEquals(" + (requsitionId + "-" + itemId) + ")").length;

                        if (duplicateItemId > 0) {
                            toastr.warning("Same item already added in purchase order.");
                            return false;
                        }
                    }
                }
                else {
                    duplicateItemId = $("#POrderProductGrid tbody tr").find("td:eq(14):contains(" + (requsitionId + "-" + itemId) + ")").length;
            ...
            $("#ContentPlaceHolder1_ddlStockBy").change(function () {
                var itemId = $("#ContentPlaceHolder1_ddlProductId").val();
                var stockId = $("#ContentPlaceHolder1_ddlStockBy").val();
                PageMethods.GetItemPrice(itemId, stockId, OnLoadItemPriceSucceeded, OnLoadItemPriceFailed);
            });
        });
        ...
        function AddPurchaseOrderItem(itemName, quantity, stockBy, purchasePrice, totalPrice, orderType, purchaseOrderDetailsId, requsitionId, categoryId, itemId, costCentreId, stockById) {
            ...
            tr += "<td style='display:none'>" + requsitionId + '-' + itemId + "</td>";
        ...
        function EditItem(itemName, quantity, stockBy, purchasePrice, totalPrice, orderType, purchaseOrderDetailsId, requsitionId, categoryId, itemId, costCentreId, stockById) {
            ...
            $(editedItem).find("td:eq(14)").text(requsitionId + '-' + itemId);
        ...
        function OnFillFormSucceed(result) {
            ...
            if (result != null) {
                ...
                if ($("#ContentPlaceHolder1_hfpurchaseOrderTemplate").val() == "1") {
                    ...
                    $('#ContentPlaceHolder1_ddlIsLocalOrForeignPO').val(result.PurchaseOrder.IsLocalOrForeignPO);
    ...
    function ValidationBeforeSave() {
        ...
        var pOrderId = "0", receivedByDate = "0", isLocalOrForeignPO = "Local", supplierId = "0", isEdited = "0";
        var categoryId = "", itemId = "", costCenterId = "", stockById = "", requsitionId = "0";
        var purchaseOrderDetailsId = "0", quantity = "0", purchasePrice = "0", remarks = "", checkedBy = '', approvedBy = '';

        pOrderId = $("#ContentPlaceHolder1_hfPOrderId").val();
        isLocalOrForeignPO = $("#ContentPlaceHolder1_ddlIsLocalOrForeignPO").val();
    ...
    function LoadProductList() {
        var ddlCostCentre = '<%=ddlCostCentre.ClientID%>'
        var costCentreId = $('#' + ddlCostCentre).val();
        var ddlCategory = '<%=ddlCategory.ClientID%>'
        var Category = $('#' + ddlCategory).val();
        var ddlPRNumber = '<%=ddlPRNumber.ClientID%>'
        var PRNumber = $('#' + ddlPRNumber).val();

        PageMethods.LoadProductListOnPONumberChange(costCentreId, Category, PRNumber, OnLoadProductListOnPONumberChangeSucceeded, OnLoadProductListOnPONumberChangeFailed);
        CommonHelper.SpinnerClose();
        return false;
    }
    ...
    function OnLoadProductListOnPONumberChangeSucceeded(result) {
        ...
        function OnSetSelected(response) {
            ...
            if ($('#ContentPlaceHolder1_ddlPRNumber').val() != "0") {
                ...
                $("#<%=txtQuantity.ClientID %>").val(response.d.Quantity - response.d.PurchaseQuantity);
                $("#ContentPlaceHolder1_ddlCostCentre").val(response.d.CostCenterId);
                $("#ContentPlaceHolder1_ddlStockBy").val(response.d.StockById);
                $("#ContentPlaceHolder1_hfCurrentLoadedItemStockId").val(response.d.StockById);
            ...
            if (productId > 0) {
                ...
                LoadItemRelatedStockByItemId(productId);
            }
        }
        ...
        function LoadItemRelatedStockByItemId(itemId) {
            PageMethods.GetItemRelatedStockByItemId(itemId, OnLoadStockBySucceeded, OnLoadStockByFailed);
        }
        function OnLoadStockBySucceeded(result) {
            ...
            var StockId = $("#ContentPlaceHolder1_hfCurrentLoadedItemStockId").val();
            $("#ContentPlaceHolder1_ddlStockBy").val(StockId);
            return false;
        }
        function OnLoadStockByFailed(error) {
        }
        function OnLoadItemPriceSucceeded(result) {
            $("#<%=txtPurchasePrice.ClientID %>").val(result.PurchasePrice);
        }
        function OnLoadItemPriceFailed(error) {
        }
        function ClearPurchaseOrder() {
            $("#ContentPlaceHolder1_hfProductId").val("0");
            $("#ContentPlaceHolder1_ddlCategory").val('0').trigger('change');
            $("#ContentPlaceHolder1_ddlProductId").val('0').trigger('change');
            $("#ContentPlaceHolder1_ddlCostCentre").val("0");
            $("#ContentPlaceHolder1_ddlStockBy").val("0");
            $('#ContentPlaceHolder1_txtPurchasePrice').val("");
            $("#ContentPlaceHolder1_txtQuantity").val("");
        ...
        function PerformClearAction() {
            ...
            $("#ContentPlaceHolder1_ddlIsLocalOrForeignPO").val("Local");
        ...
        function ValidationBeforeSave2() {
            ...
            if (IsPermitForSave == true) {
                ...
                var pOrderId = "0", receivedByDate = "0", isLocalOrForeignPO = "Local", supplierId = "0", isEdited = "0";
                var categoryId = "", itemId = "", costCenterId = "", stockById = "", requsitionId = "0", approvedBy = "0";
                var purchaseOrderDetailsId = "0", quantity = "0", purchasePrice = "0", remarks = "", checkedBy = "0";

                pOrderId = $("#ContentPlaceHolder1_hfPOrderId").val();
                isLocalOrForeignPO = "Local";
        ...
        <table id="DetailsPOGrid" class="table table-bordered table-condensed table-responsive"
            ...
            <thead>
                <tr style="color: White; background-color: #44545E; text-align: left; font-weight: bold;">
                    ...
                    <th style="width: 20%;">Purchase Price
                    </th>
    ...
    <asp:HiddenField ID="hfCurrentLoadedItemStockId" runat="server" Value="0" />
    <div id="myTabs">
            ...
            <div id="POrderTemplate1" class="panel-body" runat="server">
                <div class="form-horizontal">
                    <div class="form-group">
                        <div class="col-md-2">
                            <asp:Label ID="lblIsLocalOrForeignPO" runat="server" class="control-label required-field"
                                Text="Local/ Foreign PO"></asp:Label>
                        </div>
                        <div class="col-md-4">
                            <asp:DropDownList ID="ddlIsLocalOrForeignPO" runat="server" CssClass="form-control"
                                TabIndex="2">
                    ...
                    <div id="Commissionformation" class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <div class="form-horizontal">
                                ...
                                <div id="CategoryDiv" class="form-group" style="display:none;">
                                    <div class="col-md-2">
                                ...
                                <div class="form-group" id="POorderTableContainer">
                                    <table id="POrderProductGrid" class="table table-bordered table-condensed table-responsive"
                                        ...
                                        <thead>
                                            <tr style='color: White; background-color: #44545E; text-align: left; font-weight: bold;'>
                                                <th style="width: 24%;">Product Name
                                                </th>
                                                <th style="width: 12%;">Quantity
                                                </th>
                                                <th style="width: 15%;">Stock By
                                                </th>
                                                <th style="width: 14%;">Purchase Price
                                                </th>
                                                <th style="width: 15%;">Total Price
                                                </th>
                                                <th style="width: 12%;">Order Type
                                                </th>
                                                ...
                                                <th style="display: none">duplicate check
                                                </th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                ...
                                <div class="form-group" style="text-align: left; margin-top: 10px;">
                                    <asp:Label ID="lblTitleTotalAmount" runat="server" class="control-label" Text="Total Price :"
                    ...
                    <div id="CheckNApproveDiv" class="form-group" runat="server">
                        <div class="col-md-2">
                            <asp:Label ID="lblCheckedBy" runat="server" class="control-label" Text="Checked By"></asp:Label>
                        </div>
                        <div class="col-md-4">
