...
{
    public partial class frmReportSupplierAging : BasePage
    {
        ...
        protected void btnGenarate_Click(object sender, EventArgs e)
        {
            ...
            {
                reportPath = Server.MapPath(@"~/PurchaseManagment/Reports/Rdlc/rptSupplierAPAgingTop.rdlc");
            }
            else if (ddlReportType.SelectedValue == "2")
            {
                reportPath = Server.MapPath(@"~/PurchaseManagment/Reports/Rdlc/rptSupplierAPAgingSummary.rdlc");
            }
            else if (ddlReportType.SelectedValue == "3")
            {
                reportPath = Server.MapPath(@"~/PurchaseManagment/Reports/Rdlc/rptSupplierAPAgingDetail.rdlc");
            }
            ...
            {
                companyARAging = commonReportDa.GetSupplierAPAgingDetail(reportType, companyId, asOfDate, intervalBands, intervalType);
            }
            else
            {
                companyARAging = commonReportDa.GetSupplierAPAging(reportType, companyId, asOfDate, intervalBands, intervalType);
            }

            var reportDataSet = rvTransaction.LocalReport.GetDataSourceNames();
            rvTransaction.LocalReport.DataSources.Add(new ReportDataSource(reportDataSet[0], companyARAging));

            rvTransaction.LocalReport.DisplayName = "Supplier A/P Aging";
