...
{
    public partial class frmReportABCAnalysis : BasePage
    {
        ...
        protected void btnGenarate_Click(object sender, EventArgs e)
        {
            ...
            if (files != null)
            {
                if (files.Count > 0)
                {
                    ...
                    var reportPath = Server.MapPath(@"~/Inventory/Reports/Rdlc/rptABCAnalysis.rdlc");
                    if (!File.Exists(reportPath))
                        return;

                    rvTransaction.LocalReport.ReportPath = reportPath;
                    AllInventoryReportDA allInvReportda = new AllInventoryReportDA();
                    List<AbcAnalysisBO> abcList = new List<AbcAnalysisBO>();
                    abcList = allInvReportda.GetAbcAnalysis(dtFromDate, dtToDate, strCostCenterIds, categoryId);

                    rvTransaction.LocalReport.SetParameters(reportParam);

                    var reportDataSet = rvTransaction.LocalReport.GetDataSourceNames();
                    rvTransaction.LocalReport.DataSources.Add(new ReportDataSource(reportDataSet[0], abcList));

                    rvTransaction.LocalReport.DisplayName = "ABC Analysis";
