...
{
    public partial class frmLCApproval : System.Web.UI.Page
    {
        ...
        protected void gvOrderInfo_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                LCInformationDA orderDetailsDA = new LCInformationDA();
                ...
                if (e.CommandName == "CmdItemPOApproved")
                {
                    ...
                    if (hfIsPurchaseOrderCheckedByEnable.Value == "1")
                    {
                        ...
                        orderBO = orderDetailsDA.GetLCInformationById(POrderId);
                        if (orderBO != null)
                        {
                    ...
                    status = orderDetailsDA.UpdateLCOrderStatus(POrderId, approvedStatus, approvedBy);
        ...
        protected void gvOrderInfo_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            {
                var item = (LCInformationBO)e.Row.DataItem;
        ...
        private void LoadSearchInformation()
        {
            ...
            LCInformationDA detalisDA = new LCInformationDA();
            List<LCInformationBO> allOrderList = new List<LCInformationBO>();
            allOrderList = detalisDA.GetLCInformationBySearchCriteria(fromDate, toDate, PONumber, ddlStatus.SelectedValue);

            if (hfIsPurchaseOrderCheckedByEnable.Value == "1")
            {
                foreach (LCInformationBO row in allOrderList)
                {
                ...
                List<LCInformationBO> userWisePendingOrderList = new List<LCInformationBO>();
                List<LCInformationBO> userWiseCheckedOrderList = new List<LCInformationBO>();
                List<LCInformationBO> userWiseApprovedOrderList = new List<LCInformationBO>();
                ...
                List<LCInformationBO> userWiseOrderList = new List<LCInformationBO>();
        ...
        [WebMethod]
        public static List<LCInformationDetailBO> PerformLoadPMProductDetailOnDisplayMode(string pOrderId)
        {
            LCInformationDA orderDetailDA = new LCInformationDA();
            List<LCInformationDetailBO> orderDetailListBO = new List<LCInformationDetailBO>();
            orderDetailListBO = orderDetailDA.GetLCDetailInformationById(Int32.Parse(pOrderId));
