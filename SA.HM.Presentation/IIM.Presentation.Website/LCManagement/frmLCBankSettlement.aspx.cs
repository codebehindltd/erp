...
{
    public partial class frmLCBankSettlement : System.Web.UI.Page
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            lcInformationBO.IsLCBankSettlement = true;
            lcInformationBO.BankSettlementBy = userInformationBO.UserInfoId;

            if (btnSave.Text.Equals("Settlement"))
            {
                Boolean status = lcInformationDA.LCBankSettlementInformation(lcInformationBO, Session["GuestPaymentDetailListForGrid"] as List<GuestBillPaymentBO>);
                if (status)
                {
                    Clear();
                    txtSrcBillNumber.Text = string.Empty;
                    Session["HiddenFieldCompanyPaymentButtonInfo"] = null;
                    CommonHelper.AlertInfo(innboardMessage, "LC Bank Settlement Successfull.", AlertType.Success);
                }
                else
                {
            ...
            if (!string.IsNullOrWhiteSpace(this.txtSrcBillNumber.Text))
            {
                ...
                if (lcInformationBO != null)
                {
                    if (lcInformationBO.LCId > 0)
                    {
                        ...
                        this.ddlChequeBankId.DataSource = entityDA.GetNodeMatrixInfoByAncestorNodeId(22).Where(x => x.IsTransactionalHead == true && x.NodeId == lcInformationBO.BankAccountId).ToList();
                        this.ddlChequeBankId.DataTextField = "HeadWithCode";
                        this.ddlChequeBankId.DataValueField = "NodeId";
        ...
        private void LoadPaymentDetails(long lcId)
        {
            ...
            List<LCPaymentBO> lcPaymentBO = new List<LCPaymentBO>();
            lcPaymentBO = lcDA.GetLCTotalPaymentById(lcId);

            gvPaymentDetail.DataSource = lcPaymentBO;
            gvPaymentDetail.DataBind();

            NodeMatrixDA entityDA = new NodeMatrixDA();
            List<NodeMatrixBO> accountsTradePayableBO = new List<NodeMatrixBO>();
            accountsTradePayableBO = entityDA.GetNodeMatrixInfoByAncestorNodeId(23);

            decimal bankAdjustmentTotalAmount = 0;
            List<NodeMatrixBO> bankAdjustmentAccountsListBO = new List<NodeMatrixBO>();
            List<NodeMatrixBO> tmpBankAdjustmentAccountsListBO = new List<NodeMatrixBO>();
            foreach (LCPaymentBO row in lcPaymentBO)
            {
                tmpBankAdjustmentAccountsListBO = accountsTradePayableBO.Where(x => x.NodeId == row.AccountHeadId).ToList();
                bankAdjustmentAccountsListBO.AddRange(tmpBankAdjustmentAccountsListBO);
            }

            this.ddlLcBankAccountHead.DataSource = bankAdjustmentAccountsListBO;
            this.ddlLcBankAccountHead.DataTextField = "HeadWithCode";
            this.ddlLcBankAccountHead.DataValueField = "NodeId";
            this.ddlLcBankAccountHead.DataBind();

            ListItem itemBank = new ListItem();
            itemBank.Value = "0";
            itemBank.Text = hmUtility.GetDropDownFirstValue();
            this.ddlLcBankAccountHead.Items.Insert(0, itemBank);

            List<LCPaymentBO> lcBankPaymentListBO = new List<LCPaymentBO>();
            List<LCPaymentBO> tmpLcBankPaymentListBO = new List<LCPaymentBO>();
            foreach (NodeMatrixBO row in bankAdjustmentAccountsListBO)
            {
                tmpLcBankPaymentListBO = lcPaymentBO.Where(x => x.AccountHeadId == row.NodeId).ToList();
                lcBankPaymentListBO.AddRange(tmpLcBankPaymentListBO);
            }

            foreach (LCPaymentBO asrow in lcBankPaymentListBO)
            {
                bankAdjustmentTotalAmount = bankAdjustmentTotalAmount + asrow.Amount;
            }
            hfBankAdjustmentTotalAmount.Value = bankAdjustmentTotalAmount.ToString();
        ...
        private void LoadBank()
        {
            ...
            //this.ddlChequeBankId.DataSource = entityDA.GetNodeMatrixInfoByAncestorNodeId(22).Where(x => x.IsTransactionalHead == true).ToList();
            //this.ddlChequeBankId.DataTextField = "HeadWithCode";
            //this.ddlChequeBankId.DataValueField = "NodeId";
            //this.ddlChequeBankId.DataBind();
        ...
        public static string LoadGuestPaymentDetailGridViewByWM(string paymentDescription)
        {
            ...
            if (detailList != null)
            {
                ...
                foreach (GuestBillPaymentBO dr in detailList)
                {
                    ...
                    {
                        // It's even
                        //strTable += "<td align='left' style='width: 40%;'>" + dr.LCBankAccountHead + "</td>";
                        strTable += "<tr style='background-color:#E3EAEB;'><td align='left' style='width: 20%;'>" + dr.PaymentMode + "</td>";
                    }
                    else
                    {
                        // It's odd
                        //strTable += "<td align='left' style='width: 40%;'>" + dr.LCBankAccountHead + "</td>";
                        strTable += "<tr style='background-color:White;'><td align='left' style='width: 20%;'>" + dr.PaymentMode + "</td>";
                    }
                    strTable += "<td align='left' style='width: 45%;'>" + dr.PaymentDescription + "</td>";
        ...
        public static string PerformSaveGuestPaymentDetailsInformationByWebMethod(int lcBankAccountHeadId, string lcBankAccountHead, bool isEdit, string paymentDescription, string ddlCurrency, string currencyType, string localCurrencyId, string conversionRate, string ddlPayMode, string ddlBankId, string txtReceiveLeadgerAmount, string ddlReservationId, string ddlCashPaymentAccountHead, string txtCardNumber, string ddlCardType, string txtExpireDate, string txtCardHolderName, string txtChecqueNumber, string ddlChecquePaymentAccountHeadId, string ddlCardPaymentAccountHeadId, string ddlCompanyPaymentAccountHead, string ddlMBankingBankId, string ddlMBankingReceiveAccountsInfo, string ddlPaidByRoomId, string RefundAccountHead)
        {
            ...
            if (currencyType == "Local")
            {
                ...
                guestBillPaymentBO.DRAmount = guestBillPaymentBO.PaymentAmount;
                guestBillPaymentBO.CRAmount = 0;
            }
            else
            {
            ...
            guestBillPaymentBO.LCBankAccountHeadId = lcBankAccountHeadId;
            guestBillPaymentBO.LCBankAccountHead = lcBankAccountHead;
        ...
        [WebMethod]
        public static string LoadLCPaymentInformation(long lcId, int accountHeadId)
        {
            decimal bankContributeAmount = 0;
            LCInformationViewBO viewBo = new LCInformationViewBO();
            LCInformationDA lcDA = new LCInformationDA();

            List<LCPaymentBO> lcPaymentBO = new List<LCPaymentBO>();
            lcPaymentBO = lcDA.GetLCPaymentById(lcId);

            foreach (LCPaymentBO row in lcPaymentBO)
            {
                if (row.AccountHeadId == accountHeadId)
                {
                    bankContributeAmount = row.Amount;
                }
            }

            decimal lcBankClearenceAmount = 0;
            List<LCPaymentBO> lcLCBankClearenceBO = new List<LCPaymentBO>();
            lcLCBankClearenceBO = lcDA.GetLCBankClearenceById(lcId);

            foreach (LCPaymentBO row in lcLCBankClearenceBO)
            {
                lcBankClearenceAmount = lcBankClearenceAmount + row.Amount;
            }

            return (bankContributeAmount - lcBankClearenceAmount).ToString();
        }
    }
}