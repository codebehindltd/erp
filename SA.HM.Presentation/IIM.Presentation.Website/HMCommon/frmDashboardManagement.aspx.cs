...
{
    public partial class frmDashboardManagement : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                LoadCurrentDate();
                SetDefaulCleanTime();
                //LoadExpGroupResvArrivNtftn();
                //SaveNotificationForEffectiveBenefit();
                //GetEmpForGratuityEligibleNotification();
                //GetEmpForPFEligibleNotification();
        ...
        public static List<DailyStatisticalReportViewBO> DailyStatisticalPieChartReport()
        {
            ...
            //EmployeeYearlyLeaveDA leaveDa = new EmployeeYearlyLeaveDA();
            //List<DailyStatisticalReportViewBO> leaveInformationList = new List<DailyStatisticalReportViewBO>();
            //leaveInformationList = leaveDa.GetDepartmentWiseTotalLeaveBalance(userInformationBO.EmpId);
        ...
        public static string LoadVacantDirtyPossiblePath(string RoomNember, string PageTitle)
        {
            ...
            //list = hmCommonDA.GetCustomFieldList("VacantDirtyPossiblePath");
        ...
        public static string LoadReservedPossiblePath(string RoomNember, string PageTitle)
        {
            ...
            //reservationBO = reservationDa.GetRoomReservationInfoByRoomId(numberBO.RoomId, DateTime.Now);
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                if (list[i].DisplayText == "Details")
                {
                    //strTable += " onclick=\"return LoadReservationDetails('" + reservationBO.ReservationId + "' );\"  />";
                    strTable += " onclick=\"return LoadReservationDetails('" + calenderBO.TransectionId + "' );\"  />";
                }
                else if (list[i].DisplayText.Trim() == "Reservation")
                {
                    //strTable += " onclick=\" location.href='" + list[i].PossiblePath + "?editId=" + reservationBO.ReservationId + "';\"  />";
                ...
                {
                    //strTable += " onclick=\" location.href='" + list[i].PossiblePath + "?rsvnId=" + reservationBO.ReservationId + "&rId=" + numberBO.RoomId + "';\"  />";
        ...
        public static string LoadOccupiedPossiblePath(string RoomNember, string PageTitle)
        {
            ...
            HMCommonDA hmCommonDA = new HMCommonDA();
            //List<CustomFieldBO> list = new List<CustomFieldBO>();
            //list = hmCommonDA.GetCustomFieldList("OccupiedPossiblePath");
            //List<string[]> stringList = new List<string[]>();
            //for (int i = 0; i < list.Count; i++)
            //{
            //    string[] tokens = list[i].FieldValue.Split('~');
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                {
                    string billPreviewLink = "/HotelManagement/Reports/frmReportGuestBillInfo.aspx";
                    ...
                    if (roomAllocationBO.RoomId > 0)
                    {
                        ...
                        //strTable += " onclick=\" location.href='" + billPreviewLink + "';\"  />";
        ...
        public static string LoadExpectedDeparturePath(string RoomNember, string PageTitle)
        {
            ...
            HMCommonDA hmCommonDA = new HMCommonDA();
            //List<CustomFieldBO> list = new List<CustomFieldBO>();
            //list = hmCommonDA.GetCustomFieldList("ExpectedDeparturePath");
            //List<string[]> stringList = new List<string[]>();
            //for (int i = 0; i < list.Count; i++)
            //{
            //    string[] tokens = list[i].FieldValue.Split('~');
        ...
        public static string LoadVacantPossiblePath(string RoomNember, string PageTitle)
        {
            ...
            HMCommonDA hmCommonDA = new HMCommonDA();
            //List<CustomFieldBO> list = new List<CustomFieldBO>();
            //list = hmCommonDA.GetCustomFieldList("ReservedPossiblePath");
            //List<string[]> stringList = new List<string[]>();
            //for (int i = 0; i < list.Count; i++)
            //{
            //    string[] tokens = list[i].FieldValue.Split('~');
        ...
        public static GuestInformationBO GetRegistrationInformationForSingleGuestByRoomNumber(string roomNumer)
        {
            ...
            foreach (GuestInformationBO row in list)
            {
                ...
                //row.ArriveDate = allocationBO.ArriveDate;
        ...
        public static GuestInformationBO PerformViewActionForGuestDetail(int guestId)
        {
            ...
            //guestBO.ArriveDate = allocationBO.ArriveDate;
        ...
        public static ReturnInfo ChangeRoomStatus(int roomId, string cleanupStatus, string remarks, string fromDate, string toDate, string cleanDate, string cleanTime, string lastCleanDate)
        {
            ...
            else if (cleanupStatus == "Available")
            {
                ...
                roomNumberBO.LastCleanDate = DateTime.Now.ToString();
                //if (!string.IsNullOrWhiteSpace(lastCleanDate))
                //{
                //    roomNumberBO.LastCleanDate = lastCleanDate;
                //}
                //else
                //{
                //    roomNumberBO.LastCleanDate = DateTime.Now.ToString();
